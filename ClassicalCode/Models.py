...
class MeanField_test:
	'''
	Computes the quantities in the Mean Field approximation H = theta sum si
	'''	
	...
	def partition_function(self):
		...
		Return : partition function (scalar)
		'''
		return (2*np.cosh(self.beta*self.parameters) )**self.n_samples
	...
	def local_gradient(self, sample):
		...
		Return : gradient (1D array)
		'''
		return -self.n_samples*np.tanh(self.beta*self.parameters) - sample.sum()
